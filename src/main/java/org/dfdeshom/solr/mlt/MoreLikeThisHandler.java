...
public class MoreLikeThisHandler extends RequestHandlerBase  
{
  ...
  { 
    ...
    public Query boosted(QParser parser,Query q) throws SyntaxError {
      ...
      ValueSource vs;
      if (b == null) return q;
      Query bq = parser.subQuery(b, FunctionQParserPlugin.NAME).getQuery();
    ...
    public DocListAndSet getMoreLikeThis(QParser parser, int id, Sort lsort, int start, int rows, List<Query> filters, List<InterestingTerm> terms, int flags ) throws IOException
    {
      ...
      try
      {
        resultingQuery = boosted(parser,realMLTQuery);
      } catch(Exception e){
        // no boosting
        resultingQuery = realMLTQuery;
      }         
...
//    }
  ...
  public String getDescription() {
    return "org.dfdeshom.solr.mlt MoreLikeThis";
  }
